...
const READ = (obj) => {
  ...
  // limit: [column, ASC/DESC]
  if (obj.hasOwnProperty('order') && obj.order.length > 0) {
    var orderBy = obj.order.length === 1 ? obj.order[0] : obj.order.join(', ')
  }
  ...
  data['sql'] = `SELECT ${select} FROM ${obj.table} ${join ? 'JOIN ' + join : ''} ${where ? 'WHERE ' + where : ''} ${orderBy ? 'ORDER BY ' + orderBy : ''} ${limit ? 'LIMIT ' + limit : ''}` // Cria a instrução
  return QUERY(data)
}
